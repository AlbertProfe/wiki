...
# Testing REST APIs in Spring Boot

There are four primary methods to test a `REST API` in Spring Boot:
...
To run the H2 database in server mode and allow multiple connections, you can use the following command:
...
- [https://h2database.com](https://h2database.com/){.external target='_blank'}
...
1. Spring Boot Application **(Port 8084)**:
   The test uses port 8084 for the Spring Boot application. This is likely a custom port chosen for the test environment, different from the default `8080` to avoid conflicts with other services.

2. H2 TCP Server **(Port 8082)**:
