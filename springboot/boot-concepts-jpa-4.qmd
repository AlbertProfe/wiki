...
> In modern JPA (`Java Persistence API`), the vast majority of queries fall into (or are derived from) these categories evolved from the original `string`-based and native SQL.

These three fundamental categories

**Native SQL queries**

> Direct SQL strings executed via `EntityManager.createNativeQuery(...)` or in Spring Data `JPA` via `@Query(nativeQuery = true)`.

This is "raw" database SQL

→ full power (database-specific functions, CTEs, window functions, performance tweaks, etc.), but zero portability between databases, no type safety, and refactoring breaks easily because table/column names are hardcoded strings.

**JPQL (Java Persistence Query Language)**

> The classic string-based query language in JPA (very similar to SQL but operating on entities, attributes, and relationships instead of tables/columns).

Examples:
```sql
Static via @NamedQuery or directly via createQuery("SELECT e FROM Employee e WHERE e.salary > :min")
```

In Spring Data JPA: `@Query("SELECT e FROM Employee e WHERE e.department = ?1")` or derived query methods (`findByDepartment(...)`) that Spring parses into JPQL under the hood.

→ Very readable for simple-to-medium complexity queries, portable across databases (as long as you stay within standard JPQL), but still string-based

→ typos in entity names, properties, or JPQL syntax

→ only caught at runtime.

**Criteria API (also called typesafe Criteria queries)**

> The fully programmatic, compile-time typesafe way to build queries using `CriteriaBuilder`, `CriteriaQuery`, `Root`, `Path`, `Predicate`, etc.
Introduced in JPA 2.0 (2009) precisely to solve the runtime-error problem of string-based JPQL.

Example (very simplified):
```java
JavaCriteriaBuilder cb = em.getCriteriaBuilder();
CriteriaQuery<Employee> cq = cb.createQuery(Employee.class);
Root<Employee> emp = cq.from(Employee.class);
cq.select(emp)
  .where(cb.greaterThan(emp.get("salary"), new BigDecimal("50000")));
```
→ Errors like wrong property name (`emp.get("sallary")`) → compile-time failure (if using metamodel classes), not runtime. Also allows dynamic query construction (adding **predicates** conditionally), but is very verbose for simple queries → often not the first choice for static queries.

→ Excellent for dynamic queries (building `WHERE` clause based on user input, pagination, sorting, etc.)

→ Very verbose for simple cases → usually not the first choice for static queries
