...
Java developers have several options to create frontend applications, whether it's the **server-side** approach of `Vaadin`, the rich` client capabilities` of `JavaFX` or `Thymeleaf`, or the native look and feel of `SWT`, there is a `Java-based` frontend solution to suit various development needs.

An also pure Java `Vaadin` is a nice alternative in two flavors: `Hilla` and `Flow`.

:::

</br>

![Java Framewoks](https://hashstudioz.com/blog/wp-content/uploads/2024/01/Best-Java-Frameworks-for-Web-Development-1060x557.webp)
